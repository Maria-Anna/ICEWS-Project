...
#Create Correct borders for ggplot
borders<-borders(database = "map", xlim= c(min(data_icews_pgm$Longitude), max(data_icews_pgm$Longitude)), ylim= c(min(data_icews_pgm$Latitude), max(data_icews_pgm$Latitude)))
borders<-borders$data
unique(borders$region)
not_needed<-c("Yemen", "Uzbekistan", "Turkey", "Turkmenistan", "Syria", "Ascension Island", "Saint Helena" , "Western Sahara",
              "Qatar","Palestine","Madeira Islands", "Portugal", "Oman", "Pakistan", "Kuwait", "Jordan", "Italy" ,"Iran","Iraq",
...
#Plot with all Events across all years
#Absolute Frequency
count_events_poly_abs<-count_events_poly %>% filter(n.x> 0)
ggplot(data=map) + 
  geom_sf(col = "black", alpha = 0.00001) +
  geom_sf(data = count_events_poly_abs, aes(fill = n.x),col = "transparent") +
  borders+
  xlab("Longitude")+ylab("Latitude")+
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(color = "black", size=14, hjust=0.5),
...
count_events_poly_001<-count_events_poly %>% filter(rel > 0)

#Ggplot only events with relative frequency over 0.01
ggplot(data=map) + 
  geom_sf(col = "black", alpha = 0.00001) +
  geom_sf(data = count_events_poly_001, aes(fill = rel),col = "transparent") +
  #borders+
  xlab("Longitude")+ylab("Latitude")+
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(color = "black", size=14, hjust=0.5),
  ...
  scale_fill_viridis(option = "B","Relative Frequency",discrete = F, direction=-1, breaks=c(0,0.25,0.5,0.75,1),labels=c(">0",0.25,0.5,0.75,"1"),
                     limits=c(0,1))+
