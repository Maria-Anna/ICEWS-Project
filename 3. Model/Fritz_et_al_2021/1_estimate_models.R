...
#Remark: Fritz et al. (2021) model with MCW, with data from 1990-01-01 till 2020-08-01
...
for(i in 1:length(dates)){
  for(s in s_values) {
    ...
    #Remark: predict at country-month level prob. of fatality for e.g for 2016-10-01 with data of 2016-08-01 and so on
    
    
    try_model_2 =bam(future_ged_dummy_sb~ + s(month_id, bs="gp") + #outcome variable: dummy whether state basedfatality in prio grid-month, meaning: for 1990-01-01 the sb value is for 1990-03-01
                       factor(month) + 
    ...
    #Remark: predict at prio grid-month level (including only prio grid with country-month fatality) prob. of fatality for e.g for 2016-10-01 with data of 2016-08-01 and so on
    
    
    try_model_3 = bam(future_ged_best_sb ~ s(month_id, bs="gp") + #outcome variable: number of state-based fatalities in prio grid-month, meaning: for 1990-01-01 the sb number value is for 1990-03-01
                        factor(month) + 
    ...
    #Remark: predict at prio grid-month level (including only prio grid with prio grid_months with sb fatalitiess) the intensity of state-based conflict for e.g for 2016-10-01 with data of 2016-08-01 and so on
    ...
    all_data$pgm_calibrate_1$pred_stage_1 = all_data$cm_calibrate_1$pred_stage_1[match(all_data$pgm_calibrate_1$key_cm, #data set used: pgm_calibrate_1, only data from 2016-11-01, observations of prio-grid included with country-month observations with no fatalities
    ...
    #STEP 5.1: Save predictions of each stage in cm_data_comp and pgm_data_comp (the latter data set include the new generated variables of functions but are not "cleaned", meaning in pgm data set still prio grid included with country-months without fatalities) 
